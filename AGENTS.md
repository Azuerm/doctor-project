...
- 每个页面一个文件夹：`src/views/<name>/index.vue`。路由只定义在 `src/router/index.js`（`createWebHistory`），懒加载用 `() => import(...)`，参数路由使用 `props: true`。注意：HomeView 会跳转到 `/computer`，但该路由并不存在。
...
`vite.config.js` 注入了一个中间件，在开发环境拦截 `METHOD /api/...` 请求。数据存放在 `mock/index.js`，以 `"METHOD /path"` 为 key。新增接口时在此添加对应 key 即可。
